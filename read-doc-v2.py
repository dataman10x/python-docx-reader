...
def listDir(path, jsonName, extra):
    os.chdir(path)
    fullText = []
    data = ''
    tLen = 0
    ...
    for file in os.listdir():
        if file.endswith('.docx'):
            ...
            dataDocx, qLen = read_docx(file, extra)
...
if __name__ == '__main__':
    msg = '''
        ...
        # NOTE; Remove the last trailing seperator from the generated json file
    ...
    getExtra = input('Allow Extra fields? (Y/N):  ')
    if(getExtra == 'N'):
        extraField = FALSE
    
    listDir(dirName, "%s.json" %(fileName), extraField)
